...
//All Variables declaration and initialisation are here
...
function report(aword){
	...
	target.innerHTML='<p style="font-size:30px;line-height:40px;marginTop:-1000px" title="Game over"> '+board(score)+' <br> '+aword+' <a href=\"index.html\">Click here </a></p>';
...
function check(factor){
	...
	if((position >= (query.length)) || (position >= totalnumber)){
		if(score<=50){
			report(" You are funny:( Second chance ? ");
		}else if(score<=80){
...
// Following up with the timer and feedback after game is completed 

function timing(){
	countdown-- ; document.getElementById("chronograph").innerHTML =Math.floor(countdown/60)+" Minutes "+(countdown%60)+" s "+" Remaining to play" 
	if(countdown===0){
		if(score<=50){
			report(" You are so funny >:( Second chance ? ");
		}else if(score<=80){
			report(" Good for a beginner B) Try again? ");
		}else if(score>80){
			report(" Congratulations ;) Play again ? ");
